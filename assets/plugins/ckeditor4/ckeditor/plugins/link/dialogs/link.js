...
( function() {
    CKEDITOR.dialog.add( 'link', function( editor ) {
        ...
        var modxLinkDisplayTextUpdate = false;
        ...
        var checkModxTreeUpdate = function() {
            ...
            if ( parent.modx.tree.itemToChange != modxOldRessourceId )
            {
                ...
                if(dialog.getValueOf('info', 'linkDisplayText') == '') modxLinkDisplayTextUpdate = true;
                
                if(modxLinkDisplayTextUpdate === true) {
                    dialog.setValueOf('info', 'linkDisplayText', modxLinkTitle);
                }
                
                dialog.setValueOf('info','url','[~'+parent.modx.tree.itemToChange+'~]');
            }
        };
            ...
            for ( i = 0; i < ranges.length; i++ ) {
                ...
                if ( range.collapsed ) {
                    ...
                    text = new CKEDITOR.dom.text( data.linkText || ( data.type == 'email' ?
                        data.email.address : modxLinkTitle ), editor.document );
            ...
            contents: [ {
                ...
                elements: [ {
                    ...
                    },
                    {
                        type: 'text',
                        id: 'searchterm',
                        label: modxLang.searchLabel,
                        onKeyUp: function() {
                            var element_id = '#' + this.getInputElement().$.id;
                            var targetList = '#' + dialog.getContentElement('info', 'exam_ID').getInputElement().$.id;
                            var searchterm = jQuery(element_id).val();
                            
                            if(searchterm.length <= 2) {
                                jQuery(targetList).find('option').remove();
                                jQuery(targetList).append(jQuery('<option>', {
                                    value: 0,
                                    text : modxLang.noResults,
                                    'data-linktext' : ''
                                }));
                                return;
                            }
                            ...
                            searchDelay = setTimeout(function() {
                                jQuery.ajax({
                                    type: 'POST',
                                    url: '/assets/plugins/ckeditor4/ckeditor/plugins/link/search.php?q='+escape(searchterm),
                    ...
                    },
                    {
                        type: 'select',
                        id: 'exam_ID',
                        label: modxLang.searchResults,
                        items : [ [modxLang.noResults,''] ],
                        onChange: function(element) {
                            var element_id = '#' + this.getInputElement().$.id;
                            var el = jQuery(element_id + ' option:selected');
                            if(el.val() == null || el.val() == 0) return;
                            dialog.setValueOf('info', 'protocol', '' );
                            dialog.setValueOf('info', 'url', '[~'+ el.val() +'~]' );
                            dialog.setValueOf('info', 'linkDisplayText', el.data('linktext'));
                        },
                    ...
                    {
                        type: 'html',
                        html: '<br/>'+ modxLang.chooseResource
                    },
                    {
                        type: 'vbox',
                        id: 'urlOptions',
